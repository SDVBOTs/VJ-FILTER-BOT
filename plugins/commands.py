...
from database.ia_filterdb import col, sec_col, get_file_details, unpack_new_file_id, get_bad_files, get_file_by_video_id
...
async def start(client, message):
    ...
    # ------------------ PRIVATE START HANDLING ------------------
    if not await db.is_user_exist(message.from_user.id):
        await db.add_user(message.from_user.id, message.from_user.first_name)
        await client.send_message(LOG_CHANNEL, script.LOG_TEXT_P.format(message.from_user.id, message.from_user.mention))

    # ✅ Check if /start has parameter (video_id)
    if len(message.command) == 2:
        video_id = message.command[1]
        file = await get_file_by_video_id(video_id)
        if file:
            try:
                ...
                await message.reply(f"⚠️ Error sending file: {e}")
        else:
            await message.reply("❌ No file found for this ID.")
        return
